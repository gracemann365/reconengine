...
@RestController @RequestMapping("/api/naas")
public class NaasController {
    ...
    @Autowired
    public NaasController(EtlService etlService, ValidationService validationService, NaasDataQueue dataQueue) {
    ...
    public ResponseEntity<String> triggerFileIngestion( @RequestBody Map<String, String> payload) {
        ...
        if (lowerCaseFileName.contains("visa")) {
            ...
            dataQueue.validationToNormalizationQueue.addAll(validRecords); // Add valid records to the queue
            LOGGER.info("VISA file processing complete. Added " + validRecords.size() + " valid records to the queue.");
            ...
            dataQueue.validationToNormalizationQueue.addAll(validRecords); // Add valid records to the queue
            LOGGER.info("Switch file processing complete. Added " + validRecords.size() + " valid records to the queue.");
        } else {
