...
public class BatchPreparationService {
    ...
    @Autowired
    public BatchPreparationService(UnifiedQuantConsumer unifiedQuantConsumer,
        ...
        for (int i = 0; i < NUM_PROCESSING_UNITS; i++) {
            processingUnits[i] = new ProcessingUnit(i, escalationProducerService, matchingProducerService, meterRegistry);
        }
    }
    ...
    public void init() {
        ...
        scheduledExecutorService.scheduleAtFixedRate(this::pollAndRouteQuants,
    ...
    private void pollAndRouteQuants() {
        ...
        if (quant != null) {
            try {
                ...
                logger.error("Error routing Quant with transactionId {}: {}", quant.getTransactionId(), e.getMessage(), e);
    ...
    private class ProcessingUnit implements Runnable {
        ...
        public ProcessingUnit(int id, EscalationProducerService escalationProducerService, MatchingProducerService matchingProducerService, MeterRegistry meterRegistry) {
            this.id = id;
            this.escalationProducerService = escalationProducerService;
            this.matchingProducerService = matchingProducerService;
            this.quantsPairedCounter = Counter.builder("orchestrator.quants.paired")
                    .description("Number of Quants paired")
                    .tag("unit", String.valueOf(id))
                    .register(meterRegistry);
            this.pairingLatencyTimer = Timer.builder("orchestrator.pairing.latency")
                    .description("Latency of Quant pairing operations")
                    .tag("unit", String.valueOf(id))
                    .register(meterRegistry);
            // Schedule the flush task for this processing unit
            this.flushTask = scheduledExecutorService.scheduleAtFixedRate(this,
        ...
        public void processQuant(Quant quant) {
            try {
                ...
                if (pairingArena.containsKey(transactionId)) {
                    ...
                    pairingLatencyTimer.record(System.currentTimeMillis() - existingTimedQuant.getEntryTime(), TimeUnit.MILLISECONDS);
                    logger.info("ProcessingUnit {}: Paired Quants for transactionId {}. Paired buffer size: {}", id, transactionId, pairedQuantsBuffer.size());
                    checkAndFlush();
                } else {
                    pairingArena.put(transactionId, new TimedQuant(quant));
                    logger.info("ProcessingUnit {}: Added Quant for transactionId {} to pairing arena.", id, transactionId);
                }
            } catch (Exception e) {
                logger.error("ProcessingUnit {}: Error processing Quant with transactionId {}: {}", id, quant.getTransactionId(), e.getMessage(), e);
            }
        }
        ...
        private void checkAndFlush() {
            try {
                ...
                if (batchSize.get() >= BATCH_SIZE_THRESHOLD || (currentTime - lastFlushTime >= FLUSH_INTERVAL_MS && batchSize.get() > 0)) {
                    flushPairedQuants();
                }
            } catch (Exception e) {
        ...
        public void run() {
            try {
                ...
                long currentTime = System.currentTimeMillis();
                pairingArena.entrySet().removeIf(entry -> {
                    TimedQuant timedQuant = entry.getValue();
                    if (currentTime - timedQuant.getEntryTime() >= INCOMPLETE_PAIR_TIMEOUT_MS) {
                        logger.warn("ProcessingUnit {}: Quant with transactionId {} timed out. Escalating.", id, timedQuant.getQuant().getTransactionId());
                        escalationProducerService.escalateIncompleteQuant(new com.grace.recon.orchestrator.model.IncompleteQuant(timedQuant.getQuant(), timedQuant.getEntryTime(), "Pairing timeout"));
