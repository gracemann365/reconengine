...
public class TestDataProducer implements CommandLineRunner {
  ...
  public TestDataProducer(
      KafkaTemplate<String, Quant> kafkaTemplate,
  ...
  public void run(String... args) throws Exception {
    ...
    try {
      ...
      try (InputStream inputStream = resource.getInputStream()) {
        List<Quant> quants =
            objectMapper.readValue(inputStream, new TypeReference<List<Quant>>() {});
        ...
        for (Quant quant : quants) {
          ...
          System.out.println(
              "Sent Quant: " + quant.getTransactionId() + " from " + quant.getSourceSystem());
        }
        System.out.println(
            "Finished sending " + quants.size() + " Quants to " + UNIFIED_DTOS_INPUT_TOPIC);
      }
    } catch (Exception e) {
