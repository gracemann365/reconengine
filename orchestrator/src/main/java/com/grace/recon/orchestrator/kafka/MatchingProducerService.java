...
public class MatchingProducerService {
  ...
  @Autowired
  public MatchingProducerService(
      KafkaTemplate<String, List<QuantPair>> kafkaTemplate, MeterRegistry meterRegistry) {
    this.kafkaTemplate = kafkaTemplate;
    this.quantsSentForMatchingCounter =
        Counter.builder("orchestrator.quants.sent.matching")
  ...
  public void sendQuantPairs(List<QuantPair> quantPairs) {
    ...
    kafkaTemplate
        .send(MATCHING_INPUT_TOPIC, key, quantPairs)
        .whenComplete(
            (result, ex) -> {
              if (ex == null) {
                quantsSentForMatchingCounter.increment();
                logger.info(
                    "Successfully sent {} QuantPairs to topic {} with key {}. Offset: {}",
                    quantPairs.size(),
                ...
                logger.error(
                    "Failed to send {} QuantPairs to topic {} with key {}. Error: {}",
                    quantPairs.size(),
                    MATCHING_INPUT_TOPIC,
                    key,
                    ex.getMessage(),
