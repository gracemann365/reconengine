...
public class UnifiedQuantConsumer {
    ...
    public UnifiedQuantConsumer(MeterRegistry meterRegistry) {
        this.quantsConsumedCounter = Counter.builder("orchestrator.quants.consumed")
                .description("Number of Quants consumed from Kafka")
                .register(meterRegistry);
        this.quantsDroppedCounter = Counter.builder("orchestrator.quants.dropped")
    ...
    @KafkaListener(topics = "UnifiedDTOs_Input", groupId = "orchestrator-group", containerFactory = "kafkaListenerContainerFactory")
    public void listen(ConsumerRecord<String, Quant> record) {
        try {
            ...
            if (ingressBuffer.size() < MAX_BUFFER_CAPACITY) {
                ...
                logger.info("Consumed Quant with transactionId: {} and added to buffer. Current buffer size: {}", quant.getTransactionId(), ingressBuffer.size());
            } else {
                quantsDroppedCounter.increment();
                logger.warn("Ingress buffer is full. Dropping Quant with transactionId: {}. Consider increasing buffer capacity or processing speed.", quant.getTransactionId());
                // TODO: Implement more robust overflow handling, e.g., send to DLQ or apply backpressure
            }
        } catch (Exception e) {
            logger.error("Error consuming Kafka message from UnifiedDTOs_Input. Record offset: {}, partition: {}. Error: {}",
                    record.offset(), record.partition(), e.getMessage(), e);
            // Depending on the error, Spring Kafka's default error handler might re-attempt or move to next record.
        }
    }
