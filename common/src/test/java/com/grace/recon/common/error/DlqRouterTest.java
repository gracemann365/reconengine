...
class DlqRouterTest {
    ...
    void testRouteToDlq_fullArguments() {
        ...
        try (MockedStatic<LoggerFactory> mockedLoggerFactory = Mockito.mockStatic(LoggerFactory.class)) {
            ...
            mockedLoggerFactory.when(() -> LoggerFactory.getLogger(DlqRouter.class)).thenReturn(mockLogger);
            ...
            verify(mockLogger, times(1)).error(
                    ...
                    eq(topic), eq(key), eq(errorMessage), eq(value)
            );
        }
    }
    ...
    void testRouteToDlq_defaultErrorMessage() {
        ...
        try (MockedStatic<LoggerFactory> mockedLoggerFactory = Mockito.mockStatic(LoggerFactory.class)) {
            ...
            verify(mockLogger, times(1)).error(
                    eq("Routing message to DLQ. Original Topic: {}, Key: {}, Error: {}. Message Content: {}"),
                    eq(topic), eq(key), eq("Processing failed."), eq(value)
            );
        }
    }
}
