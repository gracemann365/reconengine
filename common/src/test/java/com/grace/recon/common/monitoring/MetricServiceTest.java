...
class MetricServiceTest {
    ...
    void testRecordTimerCallable() throws Exception {
        ...
        assertTrue(meterRegistry.get(timerName).tag("task", "callable").timer().totalTime(java.util.concurrent.TimeUnit.MILLISECONDS) >= 100);
    }
    ...
    void testRecordTimerRunnable() {
        ...
        assertTrue(meterRegistry.get(timerName).tag("task", "runnable").timer().totalTime(java.util.concurrent.TimeUnit.MILLISECONDS) >= 50);
    }
}
