...
class ValidatorTest {
    ...
    private static class LengthValidator implements Validator<String> {
        ...
        public ValidationResult validate(String target) {
            if (target == null || target.length() < minLength) {
                return ValidationResult.failure("String must be at least " + minLength + " characters long.");
            } else {
    ...
    void testValidationResultFailureSingleError() {
        ...
        assertEquals("ValidationResult{valid=false, errors=[Error message 1]} (Failure)", result.toString());
    }
    ...
    void testValidationResultFailureMultipleErrors() {
        ...
        assertEquals("ValidationResult{valid=false, errors=[Error message 1, Error message 2]} (Failure)", result.toString());
    }
